...
public class HtmlTagBuilderService {
	...
	private String buildQuestionTag(Question question,String style) {
		...
		builder.append("<label for=" + StringEscapeUtils.escapeHtml(question.getQuestionCategories().name()) + ">"
				+ StringEscapeUtils.escapeHtml(question.getQuestionCategories().name()) + "</label>");
		builder.append("/");
		builder.append("&nbsp;");
